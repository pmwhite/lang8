...
Each example tested by `build.sh` keeps its expected behavior in trailing L8
...
```sh
python3 tools/expect.py ./l8c3 .build/expect programs/examples/hello.l8
```
...
python3 tools/expect.py ./l8c3 .build/expect \
  --discover programs/examples --discover programs/callbacks
python3 tools/expect.py ./l8c1 .build/expect \
  --discover programs/examples --bootstrap-only --list
```
